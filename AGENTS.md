...
- **Render (Backend Web Service):**
  ...
  - The root `requirements.txt` is for Render and must remain untouched by any Netlify-side configuration.
...
- Rebuild command: `.venv/bin/python -m src.build_frontend` (requires only `jinja2`).
...
- `src.build_frontend` must rely exclusively on built-in Python libraries or ultra-light template engines (e.g., `jinja2`) without touching heavy external data dependencies.
